...
const ClaimRewardPage: React.FC = () => {
      ...
      try {
        ...
        const rewards = await Promise.all(
          (userWins as any[]).map(async (battleId: number) => {
            const battleDetails: BattleDetails = await contract.methods.getBattleDetails(battleId).call();

            if (!battleDetails || !battleDetails.voteCount || battleDetails.voteCount.length < 2) {
              return null;
            }

            const winningSide = battleDetails.winningSide;
            const prizePool = new BN(battleDetails.prizePool); // Use BN from bn.js
            const voteCountForWinner = new BN(battleDetails.voteCount[winningSide - 1]); // Use BN from bn.js
        ...
        const validRewards = rewards.filter((reward) => reward !== null) as ClaimableReward[];
  ...
  const handleClaimReward = async (battleId: number) => {
    ...
    try {
      const web3 = getWeb3();
      const contract = getContract(web3);
      const result = await contract.methods.claimReward(battleId).send({ from: address });

      if (result.status) {
        const updatedRewards = claimableRewards.filter((reward) => reward.battleId !== battleId);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#1e1f1e] w-96 relative">
        <Navbar />
        <h1 className="text-white text-center mt-4 bold text-2xl">Loading Rewards...</h1>
        <Bottombar />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#1e1f1e] w-96 relative">
      <div className="h-screen scrollbar-hide overflow-y-auto">
        <Navbar />
        <div>
          <h1 className="text-white text-center mt-4 bold text-2xl">Claim Your Rewards</h1>
          <section>
            {claimableRewards.length === 0 ? (
              <p className="text-white text-center mt-4">No rewards available to claim.</p>
            ) : (
              claimableRewards.map((reward) => (
                <div key={reward.battleId} className="mt-4">
                  ...
                  >
                    {claiming === reward.battleId ? "Claiming..." : "Claim Reward"}
                  </button>
                </div>
              ))
            )}
